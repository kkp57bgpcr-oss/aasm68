...
def process_token_update(message):
    ...
    bot.send_message(message.chat.id, "✅ **接口 Token 更新成功！**")
...
def handle_steps(message):
    ...
    if state['step'] == 'v_name':
        ...
        if ids:
            ...
            bot.send_document(message.chat.id, open("铭.txt", "rb"), caption=f"✅ 生成成功！余额 `{user_points[uid]}`", reply_markup=markup)
        del user_states[message.chat.id]
...
def logic_after_gen(message):
    uid = message.from_user.id
    user_points[uid] -= 100; save_points()
    msg = bot.send_message(message.chat.id, get_ui_bar(0, 100))
    threading.Thread(target=run_batch_task, args=(message.chat.id, msg.message_id, message.text.strip(), generated_cache[uid]['ids'], uid)).start()
    del user_states[message.chat.id]
