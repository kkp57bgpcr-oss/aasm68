...
def set_token_command(message):
    if message.from_user.id != ADMIN_ID: return
    msg = bot.send_message(message.chat.id, "🗝 发送新 Token:")
...
def update_token(m):
    ...
    bot.send_message(m.chat.id, "✅ Token 已实时更新！")
...
def handle_steps(message):
    ...
    if state['step'] == 'v_name':
        ...
        bot.send_message(message.chat.id, "发送身份证号列表:")
        ...
        bot.send_message(message.chat.id, "输入性别(男/女/未知):")
